...
if (!empty($categories)) {
    $wallFans = array_filter($categories, function($category) {
        return $category['category_id'] == 3; // Wall Fans category ID
    });

    if (!empty($wallFans)) {
        echo '<h1 class="wall-title">Wall Fans</h1>';
        echo '<div class="product-container">';

        $products = getProductsByCategory(3, $offset, $limit); // Adjust category ID for wall fans
        $totalProducts = getTotalProducts(3); // Total products for wall fans
