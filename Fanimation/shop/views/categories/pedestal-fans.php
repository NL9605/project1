...
if (!empty($categories)) {
    $pedestalFans = array_filter($categories, function($category) {
        return $category['category_id'] == 2; // Pedestal Fans category ID
    });

    if (!empty($pedestalFans)) {
        echo '<h1 class="ceiling-title">Pedestal Fans</h1>';
        echo '<div class="product-container">';

        $products = getProductsByCategory(2, $offset, $limit); // Adjust category ID for pedestal fans
        $totalProducts = getTotalProducts(2); // Total products for pedestal fans
