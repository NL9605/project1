...
if (!empty($categories)) {
    $ceilingFans = array_filter($categories, function($category) {
        return $category['category_id'] == 1; // Ceiling Fans category ID
    });

    if (!empty($ceilingFans)) {
        echo '<h1 class="ceiling-title">Ceiling Fans</h1>';
        echo '<div class="product-container">';

        $products = getProductsByCategory(1, $offset, $limit); // Adjust category ID for ceiling fans
        $totalProducts = getTotalProducts(1); // Total products for ceiling fans
