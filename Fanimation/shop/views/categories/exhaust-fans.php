...
if (!empty($categories)) {
    $exhaustFans = array_filter($categories, function($category) {
        return $category['category_id'] == 4; // Exhaust Fans category ID
    });

    if (!empty($exhaustFans)) {
        echo '<h1 class="exhaust-title">Exhaust Fans</h1>';
        echo '<div class="product-container">';

        $products = getProductsByCategory(4, $offset, $limit); // Adjust category ID for exhaust fans
        $totalProducts = getTotalProducts(4); // Total products for exhaust fans
