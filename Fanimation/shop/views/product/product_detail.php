...
<head>
    ...
    <style>
        ...
        .color-container {
            ...
            margin-top: 20px; /* Space above the color options */
        }
    ...
    <div class="detail-left">
        <div class="detail-images">
            ...
            if ($product) {
                ...
                if (!empty($imageUrls)) {
                    // Đường dẫn ảnh chính
                    $mainImage = 'http://localhost/Fanimation/app/' . htmlspecialchars(trim($imageUrls[0]));
                    ?>
                    ...
                    <?php if (count($imageUrls) > 1): ?>
                        <div class="thumbnails">
                            ...
                            foreach ($imageUrls as $image) {
                                $thumbnailPath = 'http://localhost/Fanimation/app/' . htmlspecialchars(trim($image));
                                ?>
                    ...
                    ?>
                    <img id="main-image" src="http://localhost/Fanimation/app/uploads/default-image.jpg" alt="No Image"
                         onerror="this.onerror=null; this.src='http://localhost/Fanimation/app/uploads/default-image.jpg';">
                    <?php
                }
            } else {
...
<div id="Review" class="tabcontent">
    ...
    <div id="reviewResults">
        ...
        <div id="existingReviews">
            <?php
            // Query existing reviews from the database
        ...
        <script>
            function selectStar(element) {
                ...
                // Update hidden input value with the rating
            ...
            document.getElementById('reviewForm').addEventListener('submit', function (event) {
                ...
                fetch('../../controllers/ReviewController.php', {
                    ...
                    .then(data => {
                        ...
                        `;
                            document.getElementById('existingReviews').insertAdjacentHTML('afterbegin', newReview);
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const productId = <?php echo json_encode($product['product_id']); ?>; // Get the product ID from PHP
        const colorContainer = document.getElementById('color-container');
        const selectedColorName = document.createElement('p'); // Create a paragraph to display the selected color name
        selectedColorName.id = 'selected-color'; // Set an ID for easy access
        colorContainer.appendChild(selectedColorName); // Append it to the color container

        // Fetch colors from the API
        fetch(`http://localhost/Fanimation/shop/api/product_colors.php?product_id=${productId}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                if (data.colors && data.colors.length > 0) {
                    data.colors.forEach(color => {
                        ...
                        colorBox.style.backgroundColor = color.hex_value; // Set the background color
                        colorBox.setAttribute('data-color', color.name); // Store color name
                        colorBox.onclick = function() {
                            // Remove the 'selected' class from all color boxes
                            document.querySelectorAll('.color-box').forEach(box => box.classList.remove('selected'));
                            // Add 'selected' class to the clicked color box
                            this.classList.add('selected');
                            // Update the selected color name display
                    ...
                    colorContainer.appendChild(message);
                }
            })
            .catch(error => {
                console.error('Error fetching colors:', error);
                const message = document.createElement('p');
                message.textContent = 'Error loading colors';
                colorContainer.appendChild(message);
            });
    });
    ...
    function addToCart(productId) {
        ...
        const colorName = selectedColorBox ? selectedColorBox.getAttribute('data-color') : 'No color selected'; // Lấy tên màu đã chọn
